...
function initialsOf(name: string, surname: string): string {
  return `${name.charAt(0)}${surname.charAt(0)}`.toUpperCase() || 'AP';
}

export default function AdminProfiloScreen() {
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
      ...
      <LinearGradient colors={['#1A0A2E', '#0D0D1A']} style={styles.header}>
        ...
        <LinearGradient colors={Gradients.primary} style={styles.avatarCircle}>
          <Text style={styles.avatarText}>{user ? initialsOf(user.name, user.surname) : 'AP'}</Text>
        </LinearGradient>
        <Text style={styles.name}>{fullName || 'Amministrazione Pubblica'}</Text>
        <Text style={styles.email}>{user?.email ?? ''}</Text>
        <View style={styles.roleBadge}>
          <Ionicons name="shield-checkmark-outline" size={14} color={Colors.accent} />
          <Text style={styles.roleText}>Amministrazione Pubblica</Text>
        </View>
      </LinearGradient>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.card}>
          {[
            ...
            { icon: 'shield-outline',   label: 'Ruolo',  value: 'Amministrazione Pubblica' },
          ].map((row, idx, arr) => (
