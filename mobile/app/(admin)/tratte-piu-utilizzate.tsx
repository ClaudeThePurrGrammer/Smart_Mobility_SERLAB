...
import type { ApiTrattaFrequenza } from '@/lib/api/types';

type VehicleType = 'scooter' | 'bike' | 'ebike' | 'car';
type PeriodMode  = 'week' | 'month' | 'year' | 'custom';

// Stessi valori di monitoraggio.tsx — 'scooter' nel DB = monopattino elettrico
...
export default function TrattePiuUtilizzateScreen() {
  const { token } = useAuth();
  ...
  const [results, setResults] = useState<ApiTrattaFrequenza[] | null>(null);
  const [error,   setError]   = useState<string | null>(null);

  const resetResults = (fn: () => void) => { fn(); setResults(null); setError(null); };

  const handlePickerChange = (_event: any, date?: Date) => {
    if (Platform.OS === 'android') setPickerTarget(null);
    if (!date) return;
    if (pickerTarget === 'from') { setCustomFrom(date); setResults(null); setError(null); }
    if (pickerTarget === 'to')   { setCustomTo(date);   setResults(null); setError(null); }
  };
  ...
  const handleCerca = async () => {
    ...
    try {
      ...
      const res = await amministrazioneApi.trattePiuUtilizzate(
        token!,
        vehicleType,
        range.from,
        range.to,
  ...
  const isCercaDisabled = periodMode === 'custom' && customFrom > customTo;
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Tratte Più Utilizzate</Text>
          <Text style={styles.subtitle}>Classifica delle tratte per tipologia di mezzo e periodo</Text>
        </View>
          ...
          {VEHICLE_OPTIONS.map(opt => {
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() => resetResults(() => setVehicleType(opt.type))}
                activeOpacity={0.75}
              >
          ...
          {PERIOD_OPTIONS.map(opt => {
            ...
            return (
              <TouchableOpacity
                key={opt.mode}
                style={[styles.chip, active && styles.chipActive]}
                onPress={() => resetResults(() => setPeriodMode(opt.mode))}
                activeOpacity={0.75}
              >
              ...
              <View style={styles.dateField}>
                ...
                >
                  <Ionicons
                    ...
                    size={17}
                    color={customFrom > customTo ? Colors.danger : Colors.accent}
                  />
        ...
        <GradientButton
          ...
          loading={loading}
          full
          style={{ marginTop: 28, opacity: isCercaDisabled ? 0.45 : 1 }}
          icon={<Ionicons name="analytics-outline" size={18} color={Colors.text} />}
        />
            ...
            <View style={styles.resultsHeader}>
              ...
              <Text style={styles.resultsHeaderText}>
                {results.length === 0
                  ...
                  : `Top ${results.length} tratt${results.length === 1 ? 'a' : 'e'}`}
              </Text>
            </View>
              ...
              <GlassCard padding={24}>
                <View style={styles.emptyState}>
                  ...
                  <Text style={styles.emptyStateSub}>
                    Prova a selezionare un periodo più ampio o una tipologia di mezzo diversa.
                  </Text>
                </View>
              </GlassCard>
            ) : (
              results.map((item, idx) => {
                const rankColor = RANK_COLORS[idx] ?? Colors.muted;
                const isTop3 = idx < 3;
                return (
                  <GlassCard key={`${item.from_addr}-${item.to_addr}-${idx}`} style={styles.trattaCard} padding={0}>
                    <View style={styles.trattaCardInner}>
                      {/* Posizione */}
                      <View style={[styles.rankBadge, isTop3 && { borderColor: rankColor }]}>
                        <Text style={[styles.rankText, isTop3 && { color: rankColor }]}>
                          {idx + 1}°
                        </Text>
                      </View>

                      {/* Tratta */}
                      <View style={styles.trattaInfo}>
                        <Text style={styles.trattaFrom} numberOfLines={1}>{item.from_addr}</Text>
                        <View style={styles.trattaArrowRow}>
                          <Ionicons name="arrow-forward" size={12} color={Colors.muted} />
                        </View>
                        <Text style={styles.trattaTo} numberOfLines={1}>{item.to_addr}</Text>
                      </View>

                      {/* Conteggio */}
                      <View style={styles.corseBadge}>
                        <Text style={styles.corseNumber}>{item.corse}</Text>
                        <Text style={styles.corseLabel}>
                          {item.corse === 1 ? 'corsa' : 'corse'}
                        </Text>
                      </View>
                    </View>
                  </GlassCard>
...
const styles = StyleSheet.create({
  ...
  resultsSection:      { marginTop: 28, gap: 10 },
  resultsHeader:       { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 4 },
  resultsHeaderText:   { color: Colors.accent, fontSize: 14, fontWeight: '700' },

  emptyState:          { alignItems: 'center', gap: 12, paddingVertical: 8 },
  emptyStateText:      { color: Colors.text, fontSize: 14, fontWeight: '600', textAlign: 'center' },
  emptyStateSub:       { color: Colors.muted, fontSize: 12, textAlign: 'center', lineHeight: 18 },

  trattaCard:          { overflow: 'hidden' },
  trattaCardInner:     { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 16 },

  rankBadge:           { width: 40, height: 40, borderRadius: 12, backgroundColor: Colors.surface, borderWidth: 1, borderColor: Colors.border, alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  rankText:            { color: Colors.muted, fontSize: 13, fontWeight: '800' },

  trattaInfo:          { flex: 1, gap: 2 },
  trattaFrom:          { color: Colors.text, fontSize: 13, fontWeight: '600' },
  trattaArrowRow:      { paddingLeft: 2 },
  trattaTo:            { color: Colors.muted, fontSize: 12, fontWeight: '500' },

  corseBadge:          { alignItems: 'center', minWidth: 44, flexShrink: 0 },
  corseNumber:         { color: Colors.accent, fontSize: 22, fontWeight: '900', lineHeight: 26 },
  corseLabel:          { color: Colors.muted, fontSize: 10, fontWeight: '600', letterSpacing: 0.3 },
});
