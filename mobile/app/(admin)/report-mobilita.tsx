...
export default function ReportMobilitaScreen() {
  const { token } = useAuth();
  ...
  const [result,  setResult]  = useState<ApiReportMobilita | null>(null);
  ...
  const isGenDisabled = periodMode === 'custom' && customFrom > customTo;
  ...
  const handleGenera = async () => {
    ...
    try {
      ...
      const res = await amministrazioneApi.reportMobilita(token!, vehicleType, range.from, range.to);
      setResult(res);
    } catch (e: any) {
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Report Mobilità</Text>
          <Text style={styles.subtitle}>Totale corse e tratte principali per tipologia e periodo</Text>
        </View>
        ...
        <GradientButton
          ...
          loading={loading}
          full
          style={{ marginTop: 28, opacity: isGenDisabled ? 0.45 : 1 }}
        ...
        {result && (
          <View style={styles.resultsSection}>
            ...
            {/* Totale corse — stessa GlassCard di Monitoraggio */}
            <GlassCard style={{ marginTop: 4 }} padding={24}>
              <View style={styles.resultHeader}>
                <Ionicons name="checkmark-circle-outline" size={20} color={Colors.success} />
                <Text style={styles.resultHeaderText}>Risultato report</Text>
              </View>
            ...
            {/* Top-3 tratte — stesse card di Tratte Più Utilizzate */}
            <View style={styles.tratteSection}>
              <View style={styles.tratteHeader}>
                <Ionicons name="trophy-outline" size={18} color={Colors.accent} />
                <Text style={styles.tratteHeaderText}>
                  {result.tratte.length === 0
                    ? 'Nessuna tratta trovata'
                    : `Top ${result.tratte.length} tratt${result.tratte.length === 1 ? 'a' : 'e'}`}
                </Text>
              </View>
                    ...
                    <GlassCard key={`${item.from_addr}-${item.to_addr}-${idx}`} style={styles.trattaCard} padding={0}>
                      <View style={styles.trattaCardInner}>
                        ...
                        <View style={styles.corseBadge}>
                          ...
                          <Text style={styles.corseLabel}>{item.corse === 1 ? 'corsa' : 'corse'}</Text>
                        </View>
                      </View>
                    </GlassCard>
                  );
...
const styles = StyleSheet.create({
  ...
  tratteSection:      { gap: 10 },
  tratteHeader:       { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 4 },
  tratteHeaderText:   { color: Colors.accent, fontSize: 14, fontWeight: '700' },
