...
const FEATURES: { icon: any; label: string; path: string }[] = [
  { icon: 'pulse-outline',     label: 'Monitoraggio',               path: '/(admin)/monitoraggio' },
  { icon: 'location-outline',  label: 'Segnala Zona',               path: '/(admin)/segnala-zona' },
  { icon: 'map-outline',       label: 'Tratte Più Utilizzate',       path: '/(admin)/tratte-piu-utilizzate' },
  { icon: 'shield-outline',    label: 'Inserisci Area Restrizione',  path: '/(admin)/inserisci-area-restrizione' },
  { icon: 'bar-chart-outline', label: 'Report Mobilità',             path: '/(admin)/report-mobilita' },
];
...
export default function MonitoraggioScreen() {
  ...
  const fullName = user ? `${user.name} ${user.surname}`.trim() : 'Amministrazione';
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.roleLabel}>Amministrazione Pubblica</Text>
        </View>

        {/* Griglia 5 funzionalità — tutte alla pari */}
          ...
          <Animated.View style={[styles.drawer, { transform: [{ translateX: drawerX }] }]}>
            <LinearGradient colors={['#13132A', '#0D0D1A']} style={styles.drawerGradient}>
              ...
              <TouchableOpacity
                ...
                onPress={() => drawerNav('/(admin)/profilo')}
                activeOpacity={0.65}
              >
