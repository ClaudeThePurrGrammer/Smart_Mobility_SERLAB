...
export default function ChiudiSegnalazioniScreen() {
  const { token } = useAuth();
  ...
  const fetchData = useCallback(async (isRefresh = false) => {
    ...
    setError(null);
    try {
      const data = await segnalazioniApi.listTutteAperte(token!);
      setItems(data);
    } catch (e: any) {
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Chiudi Segnalazioni</Text>
          <Text style={styles.subtitle}>Segnalazioni aperte · tutte le tipologie</Text>
        </View>
        ...
        {!loading && !error && items.map(item => {
          ...
          return (
            <TouchableOpacity
              key={item.id}
              activeOpacity={0.82}
              onPress={() => router.push({ pathname: '/(operatore)/segnalazione-dettaglio', params: { id: String(item.id), contesto: 'chiusura' } } as any)}
            >
